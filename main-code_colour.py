...
# label list
classes = np.unique(products['Colour'])
train_classes = np.unique(train_products['Colour'])
valid_classes = np.unique(valid_products['Colour'])
...
# it in the code 
classifier_saved = 'models/model_Adamax_colour.pth'
...
def test():
    ...
    for i in range(1,5):
        print("article: ", i)
        img_path = 'images/test-colour/test_' + str(i) + '.jpg'
        # print(img_path)
        # loading model
        cnn = CNN()
